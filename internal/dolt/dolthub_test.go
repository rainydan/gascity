...
func TestHubToken(t *testing.T) {
	...
	if got := HubToken(); got != "dhat.v1.test123" {
		t.Errorf("HubToken() = %q, want %q", got, "dhat.v1.test123")
	}

	os.Unsetenv("DOLTHUB_TOKEN")
	if got := HubToken(); got != "" {
		t.Errorf("HubToken() = %q, want empty", got)
	}
}

func TestHubOrg(t *testing.T) {
	orig := os.Getenv("DOLTHUB_ORG")
	defer os.Setenv("DOLTHUB_ORG", orig)

	os.Setenv("DOLTHUB_ORG", "bvts")
	if got := HubOrg(); got != "bvts" {
		t.Errorf("HubOrg() = %q, want %q", got, "bvts")
	}

	os.Unsetenv("DOLTHUB_ORG")
	if got := HubOrg(); got != "" {
		t.Errorf("HubOrg() = %q, want empty", got)
	}
}

func TestHubRepoName(t *testing.T) {
	tests := []struct {
		input string
		want  string
	}{
	...
	for _, tt := range tests {
		got := HubRepoName(tt.input)
		if got != tt.want {
			t.Errorf("HubRepoName(%q) = %q, want %q", tt.input, got, tt.want)
		}
	}
}

func TestHubRemoteURL(t *testing.T) {
	got := HubRemoteURL("bvts", "beads-gt")
	want := "https://doltremoteapi.dolthub.com/bvts/beads-gt"
	if got != want {
		t.Errorf("HubRemoteURL() = %q, want %q", got, want)
	}
}

func TestCreateHubRepo_Success(t *testing.T) {
	...
	// Override the API base so CreateHubRepo hits our mock server.
	orig := dolthubAPIBase
	dolthubAPIBase = server.URL
	defer func() { dolthubAPIBase = orig }()

	if err := CreateHubRepo("bvts", "beads-gt", "test-token"); err != nil {
		t.Fatalf("CreateHubRepo returned error: %v", err)
	}
}

func TestCreateHubRepo_AlreadyExists(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/database" {
			t.Errorf("expected path /database, got %s", r.URL.Path)
		}
	...
	if err := CreateHubRepo("bvts", "beads-gt", "test-token"); err != nil {
		t.Fatalf("CreateHubRepo returned error for 409 already-exists: %v", err)
	}
}
